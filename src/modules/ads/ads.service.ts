...
export class AdsService {
  constructor(
    @InjectModel(Ads) private readonly adsModel: ReturnModelType<typeof Ads>,
  ) {}
  ...
  async update(id: string, data: Ads): Promise<{ ads: Ads }> {
    try {
      const ads = await this.adsModel.findByIdAndUpdate(id, data, {
        new: true,
      });
      return { ads };
    } catch (error) {
  ...
  async getVideoAds(ip: string, type: string): Promise<{ ads: Ads[] }> {
    try {
      let iran;
      //پیدا کردن کشور بر اساس ip
      const country = geoip.lookup(ip).country;
